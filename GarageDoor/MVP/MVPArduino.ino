...
#define SERVICE_UUID                "40DB8631-DF2E-42D3-8D56-9AFD9487EA76"
...
BLEService publicService(SERVICE_UUID); // create service

// ability to read and notify by BLE
...
const int pinNumber = 13; // The output of your reed switch (black side)

void setup() {
  pinMode(pinNumber, INPUT); // Dont use pull up for 3v
  Serial.begin(9600);
  while (!Serial); // remove if you're not using serial
  ...
  BLE.setAdvertisedService(publicService);
  // add the characteristic to the service
  publicService.addCharacteristic(readCharacteristic);
  publicService.addCharacteristic(notifyCharacteristic);
  // add service
  BLE.addService(publicService);
